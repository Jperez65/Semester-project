...
Session(app)
# route redirect to the login route where it render the login page
@app.route('/')
def index():
    return redirect("/login", code=302)
# route set the session token to none in order to not give it access to other pages
@app.route("/logout")
def logout():
    session["id"] = None
    return redirect("/")
...
# route take in the data from register and add it to the database 
...
#route check for session token and takes in data and post it to the database and select it to be used on reviewer.html page
...
# // TODO need to add admisnitrator to the database and add more to the html page
...
# Route render the profile page where user can update and change their attribute in AuthorModel
@app.route('/profile',methods=['POST', 'GET'])
def profile():
    # save session token to local variable
    email = session.get("email_address")
    table_name = session.get("account_type")

    # check wether not it has access to a id if not redirect to the login page
    if not session.get("id"):
        return redirect("/login")
    if request.method == 'POST':
        print('made it here')
...
# Route manage the graded paper page where user can see their graded paper reports
...
def paper():
    ...
    if session.get('account_type') == 'author':
        ...
        graded_papers= db.engine.execute('SELECT * FROM paper, review WHERE paper.author_id = %s AND paper.paper_id = review.paper_id', author_id).fetchall()
        print(graded_papers)
